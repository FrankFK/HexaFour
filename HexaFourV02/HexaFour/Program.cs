...
internal class Program
{
    ...
    {
        // arrange
        var gameState = new GameState(4, 10);

        const int playerNumber = 1;

        gameState.SetToken(playerNumber, 3, 7);
        gameState.SetToken(playerNumber, 2, 8);
        gameState.SetToken(playerNumber, 1, 9);
        gameState.SetToken(playerNumber, 0, 10);

        bool expectedResult = true;

        // act
        bool result = gameState.PlayerHasConsecutiveTokens(playerNumber, 4);

        // assert
        if (result != expectedResult)
        {
            Console.WriteLine("Unexptected result");
